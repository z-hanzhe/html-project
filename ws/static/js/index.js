    ...
    if (ws_url = $.trim($("#inp_url").val()).toLocaleLowerCase()) {
        ...
        try {
            ws = new WebSocket($.trim($("#inp_url").val())), output("等待服务器握手包...", 1), ws.onopen = function () {
                ...
                output(chg_emoji(a.data), 0), askFocus || $("#inp_ask").focus()
            }, ws.onclose = function () {
...
function fun_sendto() {
    askFocus = !0, isopenEmoji && $("#emoji").trigger("click");
...
function ask() {
    window.open("http://43.154.164.250/?ask=" + $.trim($("#inp_ask").val()))
}
var ws, ws_url, isopenEmoji, askFocus, emoji, swfobject = function () {
    function A() {
        var a, c, d;
        if (!t) {
            try {
...
! function () {
...
}(), isopenEmoji = !1, askFocus = !1, WEB_SOCKET_SWF_LOCATION = "PLUG/WebSocketMain.swf", emoji = {
    ...
    fun_initEmoji(), fun_initWebSocket(), $("#div_msg").width(window.innerWidth - 530), $("#div_msg").height(window.innerHeight - 200), $("#inp_send").keydown(function (a) {
        return 13 == a.keyCode && a.ctrlKey ? ($("#btn_send").trigger("click"), !1) : void 0
    })
}), $(document).on("keydown", "#inp_ask", function (a) {
    if ("Enter" != a.key || a.shiftKey) {
        if ("Enter" == a.key && a.shiftKey) return !0
    } else a.preventDefault(), ask()
});